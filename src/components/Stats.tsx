...
function HabitRateRow({ habit, year, month }: { habit: Habit; year: number; month: number }) {
  ...
  const rate = monthlyCompletionRate(habit.frequency, doneDates, year, month);
  const streak = computeMonthlyStreak(habit.frequency, doneDates, year, month);
...
export function Stats({ theme, isDark, onToggleTheme }: Props) {
    ...
    <div className="flex h-full flex-col">
      <header className="shrink-0 border-b border-border bg-surface px-4 pb-3 pt-safe-top">
        <div className="flex items-center justify-between pt-4">
          <div className="flex items-center gap-2.5">
            ...
            {theme && onToggleTheme && <ThemeToggle theme={theme} isDark={isDark} onToggle={onToggleTheme} />}
          </div>
        </div>
      </header>
      ...
      <main className="scroll-area flex-1 px-4 py-4">
        ...
        <div className="lg:grid lg:grid-cols-2 lg:items-start lg:gap-4">
            ...
            <section className="mb-4 rounded-2xl bg-surface p-4 shadow-sm">
              <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                ...
                <div className="flex min-w-0 items-center gap-1 rounded-lg bg-surface-2 p-0.5">
                  <button aria-label="Previous month" onClick={() => shiftMonth(-1)} className="tap-target !min-h-8 !min-w-8 px-2 py-1 text-xs font-bold text-muted">‹</button>
                  <span className="min-w-20 px-1 text-center text-[11px] font-semibold text-ink sm:min-w-24">{selectedMonthDate.toLocaleDateString("en-US", { month: "short", year: "numeric" })}</span>
                  <button aria-label="Next month" onClick={() => shiftMonth(1)} disabled={isCurrentMonth} className="tap-target !min-h-8 !min-w-8 px-2 py-1 text-xs font-bold text-muted disabled:opacity-30">›</button>
                </div>
          ...
          {enabledStepTrackingForChart() && (
            <div className="lg:col-start-1 lg:row-start-5">
              <StepHistoryChart days={7} />
            </div>
          )}
