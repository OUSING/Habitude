...
export function Modal({ open, onClose, title, children, variant = "sheet" }: Props) {
  ...
  return createPortal(
    <div
      ...
      // The modal is portaled straight to document.body, but it's still a
      // React *child* of whatever rendered it (e.g. HabitCard/LoopHabitRow,
      // which wrap their row in SwipeToDelete). React's synthetic events
      // bubble along that component tree, not the real DOM — so without
      // this, a pointerdown on a button in here would still reach
      // SwipeToDelete's onPointerDown and get captured by the row,
      // silently breaking every button/click inside the modal. Stopping
      // propagation here keeps the modal's events local to itself.
